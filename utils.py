...
def initialization(args, model, w0decay=1.0):
    ...
    p = nn.Parameter(torch.zeros(len(w0), device=device) * torch.log(w0.abs().mean()), requires_grad=True)
    p.data[:pretrain_dim] += 2.0 * torch.log(w0_pretrain.abs().mean())
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                p.data[:self.pretrain_dim] -= 0.10 * m_t_hat[:self.pretrain_dim] / (
